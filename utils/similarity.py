...
def display_im(im: np.ndarray, im_pred: np.ndarray):
    fig, (ax1, ax2) = plt.subplots(ncols=2)
    ax1.imshow(im)
    ax2.imshow(im_pred)
    plt.show()

def compute_metrics(im: np.ndarray, im_pred: np.ndarray):
    display_im(im, im_pred)
    for k in metric_functions:
        try:
            with warnings.catch_warnings():
                warnings.simplefilter('ignore')
                print(k + ':', metric_functions[k](im, im_pred))
        except Exception as e:
            print(k, 'failed due to exception', e)
